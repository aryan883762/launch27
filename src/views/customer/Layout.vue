      ...
      <el-card class="header">
        <el-row class="flex-row-center full-height">
          ...
          <el-col :md="12" :sm="12" :xs="24" class="avatar-holder hidden-sm-and-down">
            <el-button @click="$router.push({ name: 'customer_bookings_new' })" 
                        class="last" plain round size="mini" type="primary">
              Book Now
            </el-button>
...
<!--            <el-button @click="$router.push({ name: 'dashboard_getting_started' })" class="last" round size="mini" type="primary">
      ...
      <el-container>
        <side-menu class="hidden-sm-and-down side-menu-container" v-if="$can('manage', 'plan_customer_portal')"/>
        <el-main :class="{content:true, 'mega-content-layer': true, 'main-content-noplan': !$can('manage', 'plan_customer_portal')}">
